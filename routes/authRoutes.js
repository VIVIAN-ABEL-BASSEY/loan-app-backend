...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            const {Surname, Firstname, Middlename, email, password,Phonenumber, nin } = req.body;
            let user = await User.findOne({ email });
            if (user) return res.status(400).json({ message: "User already exists" });
            const salt = await bcrypt.genSalt(10);
            const hashedPassword = await bcrypt.hash(password, salt);
            user = new User({ Surname, Firstname, Middlename, email, password: hashedPassword,Phonenumber, nin });
            await user.save();
